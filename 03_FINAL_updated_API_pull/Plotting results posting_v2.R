######################################################################################################
# Title: Cumulative Posting of Adverse Events After Journal Publication
...
#
# Data source:
#   Posting_Update_CTG_trial_level_data_v2.csv — updated ClinicalTrials.gov API pull conducted on 2026-06-29
######################################################################################################
...
plot_surv <- function(surv_df, title, output_name) {
  ...
  x_breaks_orig <- c(-365, -180, 0, 180, 365) 
  ...
  p <- ggsurvplot(
    ...
    ylab             = "Cumulative proportion with Adverse Events posted (%)",
    title            = title,
    ylim             = c(0, 1),
...
###############################
plot_surv(df_post,
          "Cumulative Posting of Adverse Events After Journal Publication",
          "Survival_results_post_date_v2.png")

###############################
# Footnotes 
###############################

# Trials with results posted >365 days prior to publication
gt365_before <- df %>%
  filter(!is.na(pub_date)) %>%
  mutate(days_before = as.numeric(results_post_best - pub_date)) %>%
  filter(!is.na(days_before) & days_before < -365) %>%
  mutate(is_act_flag = str_to_upper(as.character(is_act)) == "YES")

cat(sprintf("\nTrials with results posted >365 days prior to publication: %d\n", nrow(gt365_before)))
cat(sprintf("  ACT:     %d\n", sum(gt365_before$is_act_flag)))
cat(sprintf("  Non-ACT: %d\n", sum(!gt365_before$is_act_flag)))
cat(sprintf("  Range:   %d to %d days prior to publication\n",
            abs(min(gt365_before$days_before)),
            abs(max(gt365_before$days_before))))

# Non-ACT: % posted within 1 year and all within first 6 months
nonact_1yr <- df_post %>%
  filter(str_detect(as.character(act_label), "Non-ACT")) %>%
  summarise(
    n_total    = n(),
    n_posted   = sum(event == 1),
    pct_posted = round(100 * n_posted / n_total, 1),
    all_within_6mo = all(time[event == 1] <= 180)
  )
cat(sprintf("\nNon-ACT posted within 1 year: %d/%d (%.1f%%)\n",
            nonact_1yr$n_posted, nonact_1yr$n_total, nonact_1yr$pct_posted))
cat(sprintf("All Non-ACT postings within first 6 months: %s\n",
            nonact_1yr$all_within_6mo))
